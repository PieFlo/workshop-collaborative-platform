...
function getDataBase()
{
    ...
    try
    {
        $bdd = new PDO('mysql:host='.$host.';dbname='.$dbName.";port=".$port.';charset=utf8', $login, $password, array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
    }
    catch (Exception $e)
    {
        $bdd = null;
...
function addPublisherToCart($id)
{
    ...
    if ($publisher) {
        ...
        if (! isset($_SESSION["cart"]) || $_SESSION["cart"] == null) {
            $_SESSION["cart"] = new Cart($_SESSION['loginUser']);
        }
...
function getCommande($login, $idCmd, PDO $bdd = null)
{
    ...
    if ($bdd) {
        // connexion réussie
        // 3 - On récupère toutes les données de la commande
        $query = "SELECT date, idcmd, iditem, pub_name as libelle, quantity 
                    FROM command as c, commanddetail as cdt, publishers as p 
                    WHERE c.id = cdt.idcmd 
                        AND cdt.iditem = p.pub_id
